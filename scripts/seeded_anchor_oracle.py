"""Offline seeded instance-anchor oracle (no training, no model change).

For each GT instance, pick ONE real local unit as the instance anchor seed
(random unit inside the instance, or the instance's most central unit as an
upper bound).  The seed's DINO embedding is the prototype; all units are
hard-assigned by cosine nearest prototype; the prototype is re-estimated as
the cluster's unit mean and the assignment iterated 1/3 times.  Frozen
Gaussian geometry renders the masks, scored with the LSM protocol.

Reference: DINO+Agglomerative AP50=0.324, DINO GT-prototype (instance mean
feature) oracle AP50=0.534.  Question: does an anchor coming from a real
local unit (not the GT mean) approach 0.534?
"""
...
from diagnose_pair_class_similarity import _project_gs_labels  # noqa: E402
...
def _run_anchor_iteration(
    unit_dino: np.ndarray,
    fg: np.ndarray,
    dom_inst: np.ndarray,
    fg_ids: np.ndarray,
    seed_mode: str,
    ...
    """Hard cosine-nearest-prototype assignment with prototype re-estimation.

    Returns per-unit labels (-1 = void), foreground units only get labels.
    """
    u = unit_dino.shape[0]
    labels = np.full(u, -1, dtype=np.int64)
    proto = np.zeros((fg_ids.size, unit_dino.shape[1]), dtype=np.float64)
    for gi, gid in enumerate(fg_ids.tolist()):
        members = np.where(fg & (dom_inst == gid))[0]
        if members.size == 0:
            proto[gi] = unit_dino[rng.integers(0, u)]
            continue
        if seed_mode == "optimal":
            # Most central unit: closest (cosine) to the instance DINO mean.
            center = unit_dino[members].mean(axis=0)
            center = center / max(np.linalg.norm(center), 1e-8)
            seed = members[
                int(np.argmax(unit_dino[members] @ center))
            ]
        else:  # random
            seed = int(rng.choice(members))
        proto[gi] = unit_dino[seed]
    ...
    for _ in range(iterations):
        ...
        for gi in range(fg_ids.size):
            ...
            if members.size:
                new_proto[gi] = unit_dino[members].mean(axis=0)
            else:
...
def main() -> None:
    ...
    parser.add_argument("--workspace", default="workspace/seeded_anchor_oracle")
    parser.add_argument("--label", default="seeded_anchor")
    ...
    if not any(key.startswith("enc_dec_backbone.") for key in resume_ckpt):
        ...
        if backbone_path and Path(backbone_path).is_file():
            ...
            print(
                f"[anchor-oracle] frozen-backbone eval: loaded "
    ...
    variants = [
        ("random_iter1_s0", "random", 1, 0),
        ("random_iter1_s1", "random", 1, 1),
        ("random_iter3_s0", "random", 3, 0),
        ("optimal_iter1", "optimal", 1, 0),
        ("optimal_iter3", "optimal", 3, 0),
    ]
    ...
    for i, data in enumerate(test_loader):
        ...
        # DINO unit features (same as the oracle script).
        ...
        for vname, seed_mode, iters, seed in variants:
            rng = np.random.default_rng(seed)
            labels = _run_anchor_iteration(
                unit_dino, fg, dom_inst, fg_ids, seed_mode, iters, rng
            )
            cluster_list = []
            ...
            per_scene.setdefault(scene_name, {})[vname] = {
                "ap25": results["ap_25"],
                "ap50": results["ap_50"],
                "ap": coco_ap["ap_mean"],
        ...
        print(
            f"[anchor-oracle] {scene_name} done "
    ...
    (out_dir / "seeded_anchor_oracle.json").write_text(
        json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    (out_dir / "seeded_anchor_per_scene.json").write_text(
        json.dumps(per_scene, indent=2, ensure_ascii=False), encoding="utf-8"
    )
    print("\n[anchor-oracle] summary:")
    print(
        f"{'variant':22s} {'AP25':>6s} {'AP50':>6s} {'AP':>6s} "
        f"{'pred/gt':>10s} {'recall.5':>8s} {'merge':>6s} {'frag':>6s}"
    )
    for vname, entry in summary.items():
        print(
            f"{vname:22s} {entry['ap25']:6.3f} {entry['ap50']:6.3f} "
            f"{entry['ap']:6.3f} "
